...
function populate(connection) {
  connection.query('SELECT * FROM person', (error, results, fields) => {
    ...
    const populateItems = array => ready => {
      return utils.sequence(
        array.map(x => (readyInner, index) => {
          connection.query(array[index], (error, results, fields) => {
            if (error) throw error
    ...
    utils.sequence([
      ready => populateItems(populateSimple)(ready),
      ready => connection.query(populateCalendar, (error, results, fields) => {
        if (error) throw error
        ready()
      })
